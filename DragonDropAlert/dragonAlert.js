...
let guiOpen = false;
let displayAlertsOnScreen = true;
...
register("renderOverlay", () => {
    ...
    const options = [
        ...
        ...cats.map(cat => ({
            label: `${cat.label} Alerts: ${enabledCategories[cat.key] ? "§aENABLED" : "§cDISABLED"}`
        })),
        { label: `Screen Alerts: ${displayAlertsOnScreen ? "§aON" : "§cOFF"}` }
    ];
...
register("clicked", (mx, my, button, isDown) => {
    ...
    cats.forEach((cat, i) => {
        if (inBounds(buttonX, y + 40 + (i + 1) * (buttonH + 10), buttonW, buttonH)) {
            enabledCategories[cat.key] = !enabledCategories[cat.key];
            World.playSound("random.click", 1, 1);
            ChatLib.chat(`§9[§dDragonAlert§9] §f${cat.label} alerts are now ${enabledCategories[cat.key] ? "§aENABLED" : "§cDISABLED"}`);
        }
    });
